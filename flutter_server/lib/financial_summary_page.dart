...
class _FinancialSummaryPageState extends State<FinancialSummaryPage> {
  ...
  Widget _buildBarChart(FinancialSummary s) {
    ...
    double barWidthRatio(double val) => maxVal == 0 ? 0 : (val.abs() / maxVal);
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('Key Metrics', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w600)),
        const SizedBox(height: 12),
        _barRow('Net Worth', s.netWorth, Colors.blue, barWidthRatio(s.netWorth)),
        _barRow('Income', s.totalIncome, Colors.green, barWidthRatio(s.totalIncome)),
        _barRow('Expenses', s.totalExpenses, Colors.red, barWidthRatio(s.totalExpenses)),
      ],
    );
  }

  Widget _barRow(String label, double value, Color color, double widthRatio) {
    return Padding(
